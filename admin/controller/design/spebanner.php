<?php
class ControllerDesignSpebanner extends Controller {
	private $error = array();
	...
	public function index() {
		...
		//Проверяем существует ли таблица spebanner
		if ($this->model_design_spebanner->checkSpeBanner()) {
            $this->getList();
        } else {
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('design/spebanner/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('design/spebanner/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$banner_total = $this->model_design_spebanner->getTotalBanners();

		$results = $this->model_design_spebanner->getBanners($filter_data);
		...
		foreach ($results as $result) {
			$data['banners'][] = array(
				...
				'edit'      => $this->url->link('design/spebanner/edit', 'user_token=' . $this->session->data['user_token'] . '&banner_id=' . $result['banner_id'] . $url, true)
			);
		}
		...
		$pagination->url = $this->url->link('design/spebanner', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('design/spebanner_list', $data));
	}
    ...
    protected function addSpeTable() {
        ...
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->url->link('design/spebanner', 'user_token=' . $this->session->data['user_token'], true)
        );

        $this->model_design_spebanner->addSpeTable();

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');

        $this->response->setOutput($this->load->view('design/spebanner_add_table', $data));
    }
}