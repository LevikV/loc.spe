...
class ControllerExtensionModuleSpefeatured extends Controller {
	public function index($setting) {
		...
		//Получаем данные по продуктам вкладки 2
        if (!empty($setting['product2'])) {
            $data['tab_title2'] = $setting['tab_title2'];
        ...
        //Получаем данные по продуктам вкладки 3
        if (!empty($setting['product3'])) {
            $data['tab_title3'] = $setting['tab_title3'];

            $products = array_slice($setting['product3'], 0, (int)$setting['limit']);
		...
		if ($data['products']) {
			return $this->load->view('extension/module/spefeatured/spefeatured', $data);
		}
	}
}